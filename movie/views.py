from django.shortcuts import render
from django.http import HttpResponse
from .models import Movie
import matplotlib.pyplot as plt
import matplotlib
import io
import urllib, base64

# Create your views here.

def home(request):
    #searchTerm = request.GET.get('searchMovie')
    #movies = Movie.objects.all()
    #return render(request,"home.html",{'searchTerm':searchTerm,'movies':movies})
    searchTerm = request.GET.get('searchMovie')
    if searchTerm:
        movies = Movie.objects.filter(title__icontains=searchTerm)
    else:
        movies = Movie.objects.all()
    return render(request,"home.html",{'searchTerm':searchTerm,'movies':movies, 'name':'Daniel Arango'})
    

def about(request):
    return render(request,"about.html")

def statistics_view(request):
    matplotlib.use('Agg')
    all_movies = Movie.objects.all()
    movie_genre_count = {}
    movie_count_by_year = {}
    for movie in all_movies:
        year = movie.year if movie.year else "None"
        if movie.genre:
            genre = movie.genre.split(",")[0]
        else:
            genre = "None"
        if year in movie_count_by_year:
            movie_count_by_year[year] += 1
        else:
            movie_count_by_year[year] = 1
        if genre in movie_genre_count:
            movie_genre_count[genre] += 1
        else:
            movie_genre_count[genre] = 1

    bar_width = 0.5
    bar_positions = range(len(movie_count_by_year))

    plt.bar(bar_positions,movie_count_by_year.values(),width=bar_width,align='center')

    plt.title = 'Movies per year'
    plt.xlabel('Year')
    plt.ylabel('Number of movies')
    plt.xticks(bar_positions,movie_count_by_year.keys(),rotation=90)
    plt.subplots_adjust(bottom=0.3)
    buffer = io.BytesIO()
    plt.savefig(buffer,format='png')
    buffer.seek(0)
    plt.close()

    image_png = buffer.getvalue()
    buffer.close()
    graphic = base64.b64encode(image_png)
    graphic = graphic.decode('utf-8')

    bar_positionsgen = range(len(movie_genre_count))
    plt.bar(bar_positionsgen,movie_genre_count.values(),width=bar_width,align='center')
    plt.title = 'Movies per genre'
    plt.xlabel('Genre')
    plt.ylabel('Number of movies')
    plt.xticks(bar_positionsgen,movie_genre_count.keys(),rotation=90)
    plt.subplots_adjust(bottom=0.3)
    buffer = io.BytesIO()
    plt.savefig(buffer,format='png')
    buffer.seek(0)
    plt.close()

    image_png = buffer.getvalue()
    buffer.close()
    graphic1 = base64.b64encode(image_png)
    graphic1 = graphic1.decode('utf-8')
    return render(request,'statistics.html',{'graphic':graphic, 'graphic1':graphic1})

def signup(request):
    email = request.GET.get('email')
    return render(request,'signup.html',{'email':email})